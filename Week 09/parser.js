...
function match(element, selector){
    ...
    if(selector.charAt(0) === '#'){
        ...
        // div#box.box 只考虑了这种情况
        const selectors = getSelector(selector)
        let attrs = [element.tagName]
        let classAndId = element.attributes.filter(attr => attr.name === 'class' || attr.name === 'id') 
        if(classAndId){
            classAndId.map(attr => attrs.push(attr.value))
        }
        return selectors.every((selector, i) => selector.replace(/(\#)|(\.)/g, '') === attrs[i])
    }else {
